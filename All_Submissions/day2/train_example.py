...
# TODO
1. Autoencoder to generate synthetic data for minority classes
1a. Augmentation for minority classes
2. MLP mixer layer for better feature extraction
3. Branching networks for multi-task learning
4. Input image data to every hidden layer

--FIX IMAGE_SIZE to 400*400
--REMOVE 1 Instance of duplicate image
-- REMOVE ALL LOW VARIANCE IMAGES
...
# L2_REG = 0.0001  # NEW: L2 regularization strength
...
def read_data(num_classes, shuffle=False, augment=False):
    '''
    ...
    # if shuffle:
    #     list_ds = list_ds.shuffle(buffer_size=1000, seed=716)
    #
    # # NEW PHASE 2: Create process_path wrapper with augmentation parameter
    # if augment:
    #     process_fn = lambda x, y: process_path(x, y, augment=True)
    # else:
    #     process_fn = lambda x, y: process_path(x, y, augment=False)
    #
    # final_ds = list_ds.map(process_fn, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE)
    # final_ds = final_ds.prefetch(AUTOTUNE)
    # return final_ds

    paths = xdf_dset['id'].apply(lambda x: DATA_DIR + x).values
    labels = xdf_dset['target_class'].apply(lambda x: [int(i) for i in x.split(',')]).tolist()
    labels = np.vstack(labels)
    ds = tf.data.Dataset.from_tensor_slices((paths, labels))
    if shuffle:
        ds = ds.shuffle(1000, seed=716)
    ds = ds.map(lambda x, y: process_path(x, y, augment), num_parallel_calls=AUTOTUNE)
...
def model_definition():
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.ELU(alpha=1.0),  # NEW
        # tf.keras.layers.Dropout(0.2),  # NEW (light)
        ...
        tf.keras.layers.Dense(OUTPUTS_a, activation='softmax')
    ])
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),  # Lower LR
...
def train_func(train_ds, val_ds,class_weight_dict):
    '''
    ...
    early_stop = tf.keras.callbacks.EarlyStopping(
        monitor='val_loss',
        patience=20,  # Increased patience for Phase 2
...
def predict_func(test_ds):
    ...
    xdf_dset['results'] = final_model.predict(test_ds)
...
def main():
    ...
    # NEW PHASE 2: Extract labels for class weight computation
    x = lambda x: tf.argmax(x == class_names).numpy()
    ...
    # Create training subset WITH AUGMENTATION
    xdf_dset = train_data_shuffled.iloc[:split_idx].copy()
    train_ds = read_data(OUTPUTS_a, shuffle=True, augment=True)  # NEW: augment=True

    # Create validation subset WITHOUT AUGMENTATION
    xdf_dset = train_data_shuffled.iloc[split_idx:].copy()
    val_ds = read_data(OUTPUTS_a, shuffle=False, augment=False)

    # Train model with class weights
    history = train_func(train_ds, val_ds, class_weight_dict)
