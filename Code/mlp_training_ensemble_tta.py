...
def build_residual_mlp():
    ...
    def residual_block(x, units, block_id):
        ...
        x = layers.Dropout(0.2, name=f'res_dropout_{block_id}_1')(x)
        ...
        x = layers.Dropout(0.25, name=f'res_dropout_{block_id}_2')(x)
...
def build_autoencoder_mlp():
    ...
    mlp_dropouts = [0.2, 0.25, 0.3, 0.35, 0.4, 0.4]
...
def compile_model(model, is_autoencoder=False):
    ...
    if is_autoencoder:
        model.compile(
            ...
            loss_weights={'classification_output': 1.0, 'reconstruction_output': 0.3},
...
def train_single_model(model, train_ds, val_ds, model_name, is_autoencoder=False):
    ...
    callbacks = [
        tf.keras.callbacks.EarlyStopping(
            monitor=monitor_metric, patience=25,
